...
class Model {
     ...
     *  already been used. */
    Piece piece(int k) {
        if ((k < 0 || k >= _hand.size()) && (_hand.get(k) == null)) {
            return null;
        }
        return _hand.get(k);
    }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        ...
        _score += ((nrows * _height) + (ncols * _width)) - (nrows * ncols);
        _score += _streakLength * ((nrows * _height) + (ncols * _width));
        return _score - scoreBefore;
    }

    /** Return true iff the current hand is empty (i.e., piece(k) is null
     *  for all k). */
    boolean handUsed() {
        for (Piece piece : _hand) {
            if (piece != null) {
                return false;
            }
        }
